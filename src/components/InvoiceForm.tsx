...
export function InvoiceForm({ open, initial, onClose, onSave }: Props) {
  ...
  const isCapGood = inv.itemType === "capital_good" || !inv.itemType;
  const titlePrefix = initial ? "Edit" : "Add";
  const titleStr = isCapGood ? `${titlePrefix} capital goods invoice` : `${titlePrefix} input / service invoice`;
  ...
  const valid =
    ...
    inv.taxableValue > 0 &&
    totalGstRate(inv) > 0 &&
    (!isCapGood || (!inv.disposal.enabled || !!inv.disposal.date)) &&
    (!isCapGood || (!inv.usageChange.enabled || (!!inv.usageChange.date && !!inv.usageChange.newUsage))) &&
    (isCapGood ? creditNotes.every((cn) => cn.date && cn.taxableValue > 0 && totalGstRate(cn) > 0) : true) &&
    (isCapGood ? debitNotes.every((dn) => dn.date && dn.taxableValue > 0 && totalGstRate(dn) > 0) : true);
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid gap-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              {(["input", "service", "capital_good"] as const).map((t) => (
                <button
                  key={t}
                  type="button"
                  onClick={() => update({ itemType: t })}
                  className={`text-xs px-3.5 py-2 rounded-md border font-medium flex-1 transition-all ${
                    inv.itemType === t || (!inv.itemType && t === "capital_good")
                      ? "bg-primary text-primary-foreground border-primary shadow-sm font-semibold"
                      : "border-border hover:bg-muted/50 bg-background text-muted-foreground"
                  }`}
                >
                  {t === "capital_good" ? "Capital Good (Rule 43)" : t === "input" ? "Input (Rule 42)" : "Input Service (Rule 42)"}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <Field label={isCapGood ? "Asset description" : "Item / Service description *"}>
              <Input value={inv.assetName} onChange={(e) => update({ assetName: e.target.value })} placeholder={isCapGood ? "e.g. CNC Machine" : "e.g. Audit Fees / Raw Material"} />
            </Field>
          ...
          <div className="rounded-md bg-muted/40 p-3 text-xs space-y-1.5">
            ...
            {isCapGood ? (
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-1 font-medium">
                <span>Net ITC: <strong className="num text-foreground">{formatINR(grossTotalItc)}</strong></span>
                <span className="text-muted-foreground font-normal">claimed fully in month of purchase (Rule 42)</span>
              </div>
            )}
          </div>
